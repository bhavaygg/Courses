...
class MyLinearRegression():
    ...
    def cost_mse(self,X,y,n):
        #cost function for MSE
        return (np.sum(np.square((1/n)*(X.dot(self.theta)-y+self.c))))
    
    def grad_mse(self,X,y,n,epoch,alpha):
        #gradient descent for MSE
        temp=0
        for i in range(0,epoch):
            #temp = alpha*(X.T.dot(((1/n)*(X.dot(self.theta)-y+self.c))))
            self.theta -= alpha*(X.T.dot(((1/n)*(X.dot(self.theta)-y+self.c))))
            self.c -= alpha*(((1/n)*(X.dot(self.theta)-y+self.c)))
            #self.theta -= temp
            #print(self.theta)
            self.costs.append(self.cost_mse(X,y,n))
        return self
    ...
    def grad_rmse(self,X,y,n,epoch,alpha):
        ...
        for i in range(0,epoch):
            ...
            self.theta -= alpha*(X.T.dot(((1/n)*(X.dot(self.theta)-y+self.c))/(np.sqrt(((1/n)*(X.dot(self.theta)-y+self.c))**2))))
    ...
    def fit(self, X, y):
        ...
        n=X.shape[0]
        k=10
        self.yp = np.zeros(shape=(X.shape[0],1),dtype=np.float64)
        self.theta = np.zeros((X.shape[1],1),dtype=np.float64)
        self.c = 0
        self.costs=[]
        flag=0
        if flag==0:#random testing
            self.grad_mse(X,y,X.shape[0],10,0.001)
            ...
            for i in range(0,k):
                ...
                self.grad_rmse(X_train,y_train,X_train.shape[0],1000,0.1) # comment this to run MAE
                #self.grad_mse(X_train,y_train,X_train.shape[0],1000,0.001) # comment this to run MAE
                #self.grad_mae(X_train,y_train,X_train.shape[0],1000,1) #uncomment this to run MAE
                train_cost=self.costs[-1]
                self.graph(1000,self.costs,"Train")
                self.c=float(self.c[0])
                self.costs=[]
                self.grad_rmse(X_val,y_val,X_val.shape[0],1000,0.1)  # comment this to run MAE
                #self.grad_mse(X_val,y_val,X_val.shape[0],1000,0.001)  # comment this to run MAE
                #self.grad_mae(X_val,y_val,X_val.shape[0],1000,1)   # uncomment this to run MAE
                self.graph(1000,self.costs,"Validate")
    ...
    def boot(self,X,y):
        ...
        yp = self.predict(t)
        mean = np.round(np.mean(yp-y))
        var = np.round(np.mean((yp-y)**2))
        print(mean**2,var,self.cost_mse(t,y,X.shape[0]))
